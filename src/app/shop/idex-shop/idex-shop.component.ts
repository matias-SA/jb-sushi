...
@Component({
  selector: 'app-idex-shop',
  templateUrl: './idex-shop.component.html',
  styleUrls: ['./idex-shop.component.css'],
})
export class IdexShopComponent implements OnInit {
  constructor() {}
  ...
  addProductToCart(product) {
    product.quantity = product.pillQuantity;

    const isProductRepeated = this.cartProductList.some(
      (x) => x.productTitle === product.productTitle
    );
    if (isProductRepeated) {
      this.cartProductList = this.cartProductList.map((prod) => {
        const productRepeated = prod.productTitle === product.productTitle;
        return {
          ...prod,
          quantity: productRepeated ? product.quantity : prod.quantity,
        };
      });
    } else {
      this.cartProductList = [...this.cartProductList, product];
    }
  }
}
